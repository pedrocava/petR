---
title: "Aula 1 - Introdução e Sintaxe"
...
Ambos são objetos de classe "numeric", mas existem outras. Uma das estruturas de dados mais importantes do R são vetores. Podemos declarar vetores de várias formas, uma "simples" é usando a função ``c()``. Para saber se um objeto é vetor, podemos usar ``is.vector()``. Objetos podem ser nomeados com números, desde que não comecem com um número. Letras maísculas e minúsculas também fazem diferença.
