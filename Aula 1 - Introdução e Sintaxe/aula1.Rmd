...
A estrutura de dados mais comum no dia a dia da análise de dados é um Data Frame. Usamos a função ``data.frame()`` para gera-los. Um DF é uma coleção de vetores que admite tipos _diferentes_, então são mais flexíveis que matrizes. Na hora de declarar o DF, podemos dar nome a cada vetor. Observe que precisamos que todos os vetores do DF tenham o *mesmo* comprimento. Podemos usar a função ``length()``  para averiguar isso.
